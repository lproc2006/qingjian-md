...
- 首个独立文稿窗口会自动居中显示，双击文件后无需再手动调整窗口位置。

### 分屏不只是并排显示

左侧编辑 Markdown，右侧即时呈现最终结果。鼠标悬停或输入光标所在的源码行会在预览中以亮黄色标出；无论滚动编辑区还是预览区，另一侧都会按当前正文段落同步定位，确保两边当前显示的内容始终对应。两侧共用显示比例和基准字号，长文对照更自然。编辑中的回车次数和连续空行也会在预览中原样保留。
